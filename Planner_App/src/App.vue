...
<template>
  ...
  <div>
      <!-- React Parallel: `@click` is Vue's shorthand for `onClick`. `:disabled` is shorthand for `disabled={loading}`. -->
      <button @click="loadProjects" :disabled="is_loading">
        <!-- React Parallel: Ternary operator for button text `loading ? 'Fetching...' : 'Fetch Data from Flask'`. -->
        {{ is_loading ? 'Fetching...' : 'Fetch Data from Flask' }}
      </button>
    </div>
</template>
